...
// From https://github.com/Matthew-J-Spencer/Ultimate-2D-Controller/blob/main/Scripts/PlayerController.cs
/// <summary>
/// Hey!
...
/// </summary>
public class AdvancedPlayerController2D : MonoBehaviour, IPlayerController
{
    ...
    private void Update()
    {
        ...
        if (!isStunned) { MoveCharacter(); }    // Actually perform the axis movement

        if (isInTruckRange && UnityEngine.Input.GetButtonDown("Fire1"))
        {
    ...
    private void GatherInput()
    {
        ...
        // If we are currently facing right, and player goes left, we need to flip our sprite
        if (UnityEngine.Input.GetAxisRaw("Horizontal") < 0 && isFacingRight) { Flip(); }
        else if (UnityEngine.Input.GetAxisRaw("Horizontal") > 0 && !isFacingRight) { Flip(); }
    ...
    [SerializeField] private GameObject _jumpSFXPrefab;
    ...
    private void CalculateJump()
    {
        ...
        if (Input.JumpDown && CanUseCoyote || HasBufferedJump)
        {
            ...
            if (_jumpSFXPrefab != null) Instantiate(_jumpSFXPrefab);
